// src/pages/projects/ProjectAddPage.jsx - Single Page Creative Design
...
const ProjectAddPage = () => {
  ...
  useEffect(() => {
    ...
    if (userName) {
      setFormData(prev => ({ ...prev, developer: userName }));
    }
    fetchData();
  }, []);
  ...
  const getCurrentUser = () => {
    try {
      ...
      return { userId: 1, firstName: 'Hassan', lastName: 'Nassif', fullName: 'Hassan Nassif' };
    }
  };
    ...
    try {
      ...
      try {
        ...
        if (clientsResponse.data) {
          ...
          } else if (clientsResponse.data.items && Array.isArray(clientsResponse.data.items)) {
      ...
      // Fetch users - THIS IS CRITICAL for assignees
      try {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.assigneeIds || formData.assigneeIds.length === 0) {
      ...
      if (!result.isConfirmed) {
        // Focus on the assignee dropdown
  ...
  // src/pages/projects/ProjectAddPage.jsx - Updated createProject function
...
const createProject = async () => {
  try {
    ...
    // CRITICAL FIX: Convert assigneeIds to integers and ensure they're valid
    const assigneeIds = (formData.assigneeIds || [])
      .map(id => parseInt(id, 10))
      .filter(id => !isNaN(id) && id > 0);
    
    console.log('📋 Sending assigneeIds:', assigneeIds);
    console.log('📋 Manager:', formData.manager);
    
    // Make sure manager is properly set
    const managerName = formData.manager?.trim() || 'Unassigned';
    
    const projectData = {
      projectName: formData.projectName.trim(),
      clientId: parseInt(formData.clientId, 10),
      developer: formData.developer || currentUserName || 'Hassan Nassif',
      manager: managerName, // This should be saved
      ...
      assigneeIds: assigneeIds, // This MUST be an array of integers
    };
    ...
    await Swal.fire({
      ...
      html: `
        <div class="text-left">
          <p><strong>${projectData.projectName}</strong> has been created successfully.</p>
          <p class="mt-2">👤 Manager: <strong>${managerName}</strong></p>
          <p>👥 Team Members Assigned: <strong>${assigneeCount}</strong></p>
          ${assigneeCount === 0 ? '<p class="text-yellow-500 mt-2">⚠️ No team members were assigned. You can add them later.</p>' : ''}
        </div>
      `,
      icon: assigneeCount > 0 ? 'success' : 'warning',
      background: isDark ? '#141c2b' : '#ffffff',
      color: isDark ? '#e8edf5' : '#0f172a',
      confirmButtonColor: '#3b82f6',
      timer: 3000,
